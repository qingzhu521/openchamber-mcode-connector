 ...
 * OpenChamber reads for the M1 chat loop are populated. (Same contract as
 * openchamber-pi's types.ts.)
 */
...
/**
 * Tool call part (M2). One per mcode tool_call item, addressed by item.id.
 * `state` mirrors @opencode-ai/sdk v2 (1.18.31) `ToolState` exactly — a
 * discriminated union on `.status` with input/output/title living inside the
 * state object, verified against dist/v2/gen/types.gen.d.ts.
 */
...
export interface OCToolPart {
  ...
  messageID: string;
  type: "tool";
  /** Wire call id from mcode (toolCall.id); item.id is used as fallback. */
  callID: string;
  tool: string;
  state: OCToolState;
}
